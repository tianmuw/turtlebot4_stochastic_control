...
class TBController(Node):
    ...
    def __init__(self, name):
        ...
        with open('/turtlebot4/yaml/turtlebot_param.yaml', 'r') as f:
        ...
        # Subscriptions:
        self.sub_marvelmind = self.create_subscription(HedgePosition, '/hedgehog_pos', self.marvelmind_pos_callback, qos_profile_sensor_data)

        # Action client
        self.navigate_client_node = TurtleBot4ClientNode('navigate_client')
    ...
    def control_callback(self):
        while np.linalg.norm(pose_to_numpy(self.pose_current) - x_goal) > thershold:
            pose_next = stochastic_control(pose_to_numpy(self.pose_current), x_goal, x_obs, d, theta)
